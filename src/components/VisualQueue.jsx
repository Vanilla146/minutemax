    ...
    return (
        <div className="visual-queue">
            ...
            <div className="queue-visualization">
                {/* Service Counter */}
                <motion.div
                    className={`service-counter ${isYourTurn ? 'active' : ''}`}
                    animate={isYourTurn ? {
                        scale: [1, 1.08, 1],
                        boxShadow: [
                ...
                >
                    <motion.div
                        ...
                        animate={isYourTurn ? { rotate: [0, 10, -10, 0] } : {}}
                        transition={{ duration: 0.5, repeat: Infinity }}
                    >
                        {queueType === 'fitting_room' ? '🚪' : '💳'}
                    </motion.div>
                    <span>{t('queue.nowServing', 'Now Serving')}</span>
                </motion.div>
                ...
                <div className="queue-line">
                    <AnimatePresence mode="popLayout">
                        {queuePeople.map((person, index) => (
                            <motion.div
                                key={person.id}
                                className={`queue-person ${person.isYou ? 'is-you' : ''} ${person.isServed ? 'served' : ''}`}
                                variants={personVariants}
                                initial="initial"
                                animate={person.isServed ? "served" : "animate"}
                                exit="exit"
                                custom={{ index, isServed: person.isServed }}
                                layout
                            >
                                <motion.div
                                    className="person-icon"
                                    variants={walkingVariants}
                                    animate={!person.isServed && !person.isYou ? "walking" : "idle"}
                                ...
                                {!person.isYou && !person.isServed && (
                                    <span className="person-number">#{person.id}</span>
                                )}
                            </motion.div>
                        ))}
                    </AnimatePresence>
